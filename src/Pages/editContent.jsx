...
export default function EditContent() {
    ...
    const handleIdSerie = (event) => {
        setIdSerie(event.target.value);
    };
    ...
    const handlewatchOrder = (event) => {
        setWatchOrder(event.target.value);
    };
    const handlewatchStatus = (event) => {
        setWatchStatus(event.target.value);
    };
    const handleduration = (event) => {
        setDuration(event.target.value);
    };
    const handlechapters = (event) => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!watchOrder.length) {
            bodyData["watch_order"] = null;
        } else {
            bodyData["watch_order"] = watchOrder;
        }

        if (!chapters.length) {
            bodyData["chapters"] = null;
        } else {
            bodyData["chapters"] = chapters;
        }   
        if (!review.length) {
            bodyData["review"] = null;
        } else {
            bodyData["review"] = review;
        }

        const response = await fetch(`${url_api}/contents/${id}`, {
            method: "PUT",
            headers: {
            ...
            <div className="sectionAdd">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="dataToModify">
                        ...
                        <div className="formGroup">
                            <label htmlFor="seasonsInp">
                                Number of chapters (optional):{" "}
                            </label>
                            <input
                                type="text"
                                value={chapters}
                        ...
                        <div className="formGroup">
                            ...
                            <select
                                ...
                                name="Watch Status"
                                id="watchStatusInp"
                                onChange={handlewatchStatus}
                                required
                            >
                                {options_watch_status.map(({ value, label }) => {
                                    return (
                                        <option key={value} value={value}>
                                            {label}
                                        </option>
                    ...
                    <div className="btns-edit-container">
                        <button type="submit" className="btns-edit btn-confirm">
                            Confirm
                        </button>
